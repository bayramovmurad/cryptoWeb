...
export default function Page() {
  const { coins, loading, error, reload } = useCryptoPrices(60000);
    ...
    <main className="min-h-screen p-8 bg-[#0b1020] text-white">
      ...
      <section className="mb-6 p-6 flex justify-between items-center gap-6 rounded-[20px] border border-white/10 bg-[rgba(18,25,45,0.86)] backdrop-blur shadow-xl max-md:flex-col max-md:items-start">
        <div>
          ...
          <p className="text-[#b8c0d9]">Fast search, real-time updates.</p>
        </div>

        <div className="p-4 rounded-[18px] bg-linear-to-br from-blue-600 to-purple-600 text-center">
          <span className="block text-sm">Refresh</span>
          <strong className="text-2xl">60s</strong>
        </div>
      </section>
          ...
          <div className="p-5 rounded-[20px] border border-white/10 bg-[rgba(18,25,45,0.86)]">
            <div className="flex justify-between mb-4">
              ...
              <button
                onClick={reload}
                disabled={loading}
                className="px-4 py-2 rounded-xl bg-linear-to-r from-blue-600 to-purple-600 disabled:opacity-50">
